#importa tudo que tem em modles e view
from models import *
from view import *
...
class UI:
    ...
    def _criar_conta(self):
            print("Digite o nome de algum dos bancos abaixos")
           
            for banco in Bancos:
                print(f'---{banco.value}---')
            #o .title faz com que sempre fique no formato primeiro letra maiuscula e o resto minusculo, como ta os bancos (Nubank etc)  
            banco = input().title()
            print(banco)
            valor = float(input("Digite o valor da em sua conta: "))
    ...
    def _transferir_saldo(self):
        ...
        conta_retirar_id = int(input("escolha a conta para retirar dinheiro: "))
    ...
    def _movimentar_dinheiro(self):
        ...
        tipo = input().title()
        historico = Historico(conta_id=conta_id, tipo=Tipo(tipo), valor=valor, data=date.today())
    ...
    def _criar_grafico(self):
        #cria por si só não precisa de nada
